...
/*   Updated: 2023/11/21 16:12:14 by almarcos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rotate_x(t_fdf *fdf, t_point *start, t_point *end, float angle)
{
	...
	new_start.z = start->y * sin_angle + (start->z * fdf->cam->z_scale_default)
		* cos_angle;
	new_end.y = end->y * cos_angle - (end->z * fdf->cam->z_scale_default)
		* sin_angle;
	new_end.z = end->y * sin_angle + (end->z * fdf->cam->z_scale_default)
		* cos_angle;
	start->y = new_start.y;
	start->z = new_start.z;
	end->y = new_end.y;
